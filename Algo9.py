...
class Algo9(TradeChecker):
    ...
    def run(self,buyhunger=0,sellhunger=0,showgraph=False,endIndex=0):
        ...
        self.setup_logger('algologger9', 'logs/algo9.txt', level=logging.DEBUG)
        algologger = logging.getLogger('algologger9')
        ...
        while startIndex <= last:
            ...
            for asset in assets:
                ...
                atr = talib.ATR(asset.series.High.values,asset.series.Low.values,asset.series.Close.values,timeperiod=14)
                #print("asset: %s    atr: %0.2f" % (asset.name, atr[index]))
                ...
                if asset.stockcount > 0:
                    stop = close[index] - (atr[index] * 3)

                    if stop > asset.minstop:
                        asset.minstop = stop
                else :
                    asset.minstop = 0.0
                ...
                if self.capital >= min_amount and order_limit >= min_amount:
                    if close[index] > sma200[index] and close[index] > sma20[index] and close[index] > asset.minstop:
                        asset.buynextday = 1
                ...
                # SELL conditions
                if close[index] < asset.minstop and asset.stockcount > 0 and asset.nosell == False:
                    asset.sellnextday = 1
                    asset.sellprice = close[index] + (sellhungerrate * step)
                    algologger.debug(
                        "Sell-2: %s %s %0.2f" % (asset.name, asset.series.Date.values[index], asset.sellprice))
                    #print("[%s]: prev: %0.02f close: %0.02f minstop: %0.02f  sma200: %0.02f" % (asset.name, close[index - 1], close[index],asset.minstop, sma200[index]))
        ...
        final = "Algo9[sma20/200 kind]: Cap: %0.2f    MiS: %0.2f   Live P/L:%0.2f  \n" \
                "Algo9[sma20/200 kind]: Total P/L: %0.2f  Max: %0.2f Min: %0.2f \n" \
                "Algo9[sma20/200 kind]: Total: %0.2f \n" % (self.capital,
                                                            self.moneyInStock,
                                                            livepandl,
                                                            pl,
                                                            maxval, minval,
