...
class Algo10(TradeChecker):
    ...
    def run(self,buyhunger=0,sellhunger=0,showgraph=False,endIndex=0):
        ...
        self.setup_logger('algologger10', 'logs/algo10.txt', level=logging.DEBUG)
        algologger = logging.getLogger('algologger10')
        ...
        while startIndex <= last:
            ...
            for asset in assets:
                ...
                ema20 = talib.EMA(asset.series.Close.values, timeperiod=20)
                ema200 = talib.EMA(asset.series.Close.values, timeperiod=200)
                close = asset.series.Close.values
                ...
                if self.capital >= min_amount and order_limit >= min_amount:
                    if close[index] > ema200[index] and close[index] > ema20[index]:
                        asset.buynextday = 1
                ...
                # SELL conditions
                if close[index] < ema200[index] and asset.stockcount > 0 and asset.nosell == False:
                    asset.sellnextday = 1
        ...
        algologger.info("==Name=====Count=====Amount=====Mean=====Prev======Close======Ema20=====Ema200====PrevEma200========P/L=======Past-P/L=====Days4BreakEven=====PercentageFromEma200===")
        livepandl = 0.0
        ...
        for asset in assets:
            ...
            if asset.stockcount > 0:
                ema_change = ema200[-1] - ema200[-2]
                if ema_change != 0.0:
                    days_to_profit = (mean - ema200[-1])//ema_change
                else:
                    days_to_profit = 1000
            else:
                days_to_profit = 0

            uppercent = ((close[-1] - ema200[-1])/(ema200[-1]))*100
            #print(asset.name)
            algologger.info("[%5s]: %6d  %8d    %7.2f    %6.2f    %6.2f     %6.2f     %7.3f     %7.3f    %9.2f   %8.2f    %10.02d     %14.2f     %5s" %
                            (asset.name, asset.stockcount, asset.stockcount * close[-1],
                             mean, close[-2], close[-1], ema20[-1], ema200[-1], ema200[-2],
        ...
        final = "Algo10[ema20/200 kind]: Cap: %0.2f    MiS: %0.2f   Live P/L:%0.2f  \n" \
                "Algo10[ema20/200 kind]: Total P/L: %0.2f  Max: %0.2f Min: %0.2f \n" \
                "Algo10[ema20/200 kind]: Total: %0.2f \n" % (self.capital,
                                                            self.moneyInStock,
                                                            livepandl,
                                                            pl,
                                                            maxval, minval,
